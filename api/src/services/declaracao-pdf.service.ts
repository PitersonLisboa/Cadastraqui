...
export async function gerarPdfDeclaracoes(
  ...
  return new Promise((resolve, reject) => {
    ...
    function addFooter() {
      ...
      doc.image // Logo placeholder
      doc.text('CADASTRAQUI', 50, bottom, { continued: true })
      doc.text(`Pág. ${pageNum}/${totalPages}`, 50, bottom, { align: 'right' })
      doc.fillColor('#000')
    }
    ...
    const respLegal = '' // Se houver responsável legal, preencher aqui
    ...
    if (ausencia && ausencia.resposta === false) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE AUSÊNCIA DE RENDA (DESEMPREGADO(A) OU DO LAR)', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, inscrito(a) no CPF ${fmtCpf(candidato.cpf)}, declaro não exercer nenhuma atividade laboral.`)
      doc.moveDown(1)
    }

    // === DECLARAÇÃO DE AUTÔNOMO/RENDA INFORMAL ===
    const autonomo = declMap.get('AUTONOMO_INFORMAL')
    if (autonomo && autonomo.resposta === true) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE AUTÔNOMO(A)/RENDA INFORMAL', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, portador(a) do CPF nº ${fmtCpf(candidato.cpf)}, ` +
              `desenvolvo atividades ${autonomo.dados?.atividade || ''}.`)
      doc.moveDown(1)
    }

    // === DECLARAÇÃO DE RENDA DE EMPRESÁRIO ===
    const empresario = declMap.get('EMPRESARIO')
    if (empresario && empresario.resposta === true) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE RENDA DE EMPRESÁRIO', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, portador(a) do CPF nº ${fmtCpf(candidato.cpf)}, ` +
    ...
    if (empresaInativa && empresaInativa.resposta === true) {
      ...
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE EMPRESA INATIVA', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, portador(a) do CPF nº ${fmtCpf(candidato.cpf)}, ` +
    ...
    if (isentoIr && isentoIr.resposta === true) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE ISENTO DE IMPOSTO DE RENDA', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
    ...
    if (mei && mei.resposta === true) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE RENDIMENTOS – MEI', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, portador(a) do CPF nº ${fmtCpf(candidato.cpf)}, ` +
    ...
    if (separacao && separacao.resposta === true) {
      ...
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE SEPARAÇÃO DE FATO (NÃO JUDICIAL)', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Me separei de ${sep.nome || ''}, inscrito(a) no CPF nº ${sep.cpf || ''}, ` +
    ...
    if (pensao) {
      ...
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE PENSÃO ALIMENTÍCIA', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
    ...
    if (rural && rural.resposta === true) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE TRABALHADOR(A) RURAL', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, portador(a) do CPF nº ${fmtCpf(candidato.cpf)}, ` +
    ...
    if (solteiro && solteiro.resposta === true) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE ESTADO CIVIL SOLTEIRO(A)', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Eu, ${candidato.nome}, inscrito(a) no CPF ${fmtCpf(candidato.cpf)} declaro que sou solteiro(a).`)
      doc.moveDown(1)
    }
    ...
    if (uniao && uniao.resposta === true) {
      const u = uniao.dados || {}
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE UNIÃO ESTÁVEL', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
        .text(`Convivo em União Estável com ${u.parceiro || ''}, CPF ${u.cpf || ''}, ` +
    ...
    if (veiculo) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE PROPRIEDADE DE VEÍCULO AUTOMOTOR', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
    ...
    if (conta) {
      doc.fontSize(12).font('Helvetica-Bold')
        .text('DECLARAÇÃO DE ABERTURA E MANUTENÇÃO DE CONTA CORRENTE E/OU POUPANÇA', { align: 'center' })
      doc.moveDown(0.5)
      doc.fontSize(10).font('Helvetica')
    ...
    // === LGPD ===
    doc.fontSize(12).font('Helvetica-Bold')
      .text('TERMO DE CONSENTIMENTO PARA TRATAMENTO DE DADOS PESSOAIS', { align: 'center' })
    doc.moveDown(0.5)
    doc.fontSize(10).font('Helvetica')
      .text(
    ...
    doc.fontSize(12).font('Helvetica-Bold')
      .text('ALTERAÇÃO NO TAMANHO DO GRUPO FAMILIAR E/OU RENDA', { align: 'center' })
    doc.moveDown(0.5)
    doc.fontSize(10).font('Helvetica')
      .text(
    ...
    doc.fontSize(12).font('Helvetica-Bold')
      .text('INTEIRA RESPONSABILIDADE PELAS INFORMAÇÕES CONTIDAS NESTE INSTRUMENTO', { align: 'center' })
    doc.moveDown(0.5)
    doc.fontSize(10).font('Helvetica')
      .text(
